ALTER TABLE public.app_settings
  ADD COLUMN IF NOT EXISTS metaapi_symbol_suffix text NOT NULL DEFAULT '';
...
CREATE OR REPLACE FUNCTION public.get_app_settings_public()
 RETURNS TABLE(
   ...
   key1_exhausted_at timestamp with time zone, 
   ...
   metaapi_connected_at timestamp with time zone, 
   ...
   bridge_last_seen timestamp with time zone, 
...
AS $function$
  SELECT
    ...
    s.metaapi_max_trades, 
    s.metaapi_expiry_hours, 
    s.metaapi_max_daily_loss_pct,
    s.metaapi_risk_per_trade_pct, 
    s.metaapi_min_lot, 
    s.metaapi_max_lot,
